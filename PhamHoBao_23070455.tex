...
% -------------------------------------------------------
\subsection{Implementation and Qualitative Results}
...
To truly appreciate the architectural difference between a blind linear blur and an edge-preserving filter, observing a zoomed-in topological boundary is critical. The edge preservation comparison in Figure~\ref{fig:edge_preservation} explicitly shows how the Bilateral filter erects a mathematical barrier at color thresholds to keep edges sharp, while standard Gaussian convolution bleeds over the boundary.
...
The pipeline was also verified on a secondary test image, confirming that these mathematical behaviors remain consistent regardless of the underlying photographic structure (Figure~\ref{fig:img2_gaussian}).
...
\begin{itemize}
    ...
    \item \textbf{The Bilateral Paradox:} While the edge-preserving Bilateral filter achieved highly competitive PSNRs (\textbf{28.99~dB} on Image~1; \textbf{29.31~dB} on Image~2), it actually slightly underperformed the standard Gaussian filter's maximum SSIM for this specific dataset. This is because the continuous, non-zero variance of Gaussian noise frequently tricks the Bilateral's ``intensity threshold'' into believing the dense noise grain \textit{itself} is an edge, causing it to block smoothing in heavily corrupted regions.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{For Impulse Noise (Salt \& Pepper):} Scaling $k$ upward actively \textit{degrades} performance. We observed the Median filter's SSIM drop from an optimal \textbf{0.898} ($k=3$) down to \textbf{0.795} ($k=7$) in Image~2. Because the Median logic replaces the center with a ranked neighbor, expanding the search grid arbitrarily forces pixels from completely different geometric structures (like backgrounds intruding into foregrounds) to replace the center pixel, aggressively warping the natural geometry.
\end{enumerate}

\subsubsection{Part A Conclusion}
...
First, we established the epipolar geometry and rectified the stereo pair to guarantee horizontal alignment.
...
\begin{figure}[H]
    ...
    \caption{Colorized disparity map from Block Matching (BM). Warm colors indicate nearby objects (high disparity); cool colors represent distant objects.}
...
\begin{figure}[H]
    ...
    \caption{Colorized disparity map from Semi-Global Block Matching (SGBM).}
...
\begin{itemize}
    \item \textbf{Textureless regions:} In areas with minimal intensity variation (blank walls, uniform sky), the cost function $C_{SAD}$ is flat across all candidate disparities, producing essentially random disparity assignments. This manifests as the characteristic dense ``speckle noise'' visible in the BM disparity map.
...
\begin{equation}
    E(D) = \sum_{p} \left[ C(p, D_p) + \sum_{q \in N_p} P_1 \cdot \mathbb{1}[|D_p - D_q| = 1] + \sum_{q \in N_p} P_2 \cdot \mathbb{1}[|D_p - D_q| > 1] \right]
\end{equation}
...
\subsubsection{Part B Conclusion}
...
Notice how SIFT densely populates the image with gradient-based extrema, while ORB focuses sparsely on high-contrast corners (Figures~\ref{fig:keypoints_sift}--\ref{fig:keypoints_orb}).
...
Below are the matched features between overlapping views using both SIFT and ORB matchers.
...
\begin{itemize}
    \item \textbf{Adaptive Filtering:} Upgrading from statically-sized kernels to spatially adaptive filters (e.g., Adaptive Wiener Filters) that automatically compute local signal variance $\sigma^2$ before blurring, thereby providing mathematically optimal noise suppression per pixel.
